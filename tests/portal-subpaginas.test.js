...
describe('portal separado por subpáginas', () => {
  ...
  it('organiza configuraciones en solapas accesibles sin duplicar sus acciones', () => {
    ...
    expect(configuracion).toContain('name="hora_notificacion_previa"')
    expect(configuracion).toContain('data-confirmar-notificaciones')
    expect(configuracion).toContain('name="zona_horaria_notificaciones"')
  ...
  it('mantiene el encabezado fijo y el control lateral centrado', () => {
    const estilos = leer('src/styles/portal.css')
    const estilosGlobales = leer('src/styles/global.css')
    const pagina = leer('src/pages/app.js')
    ...
    ;['app.html', 'configuracion.html', 'correos.html', 'vencimientos.html', 'agenda.html', 'reglas.html'].forEach((archivo) => {
      ...
      expect(contenido).toContain('data-icon="manage_search"')
      expect(contenido).toContain('data-icon="mark_email_read"')
      expect(contenido).toContain('data-icon="settings"')
    ...
    expect(pagina).toContain("inicializarColapsoLateral({ clave: 'agenkin_menu_lateral_colapsado' })")
    const sidebar = leer('src/components/sidebar.js')
    expect(sidebar).toContain("'barra-lateral--colapsada'")
    expect(sidebar).toContain("icono.textContent = colapsado ? 'chevron_right' : 'chevron_left'")
    ...
    expect(estilosGlobales).toContain('material-symbols-rounded.woff')
    expect(estilosGlobales).toContain('Material Symbols Rounded')
    expect(estilos).toContain('content: attr(data-tooltip)')
    expect(estilos).toContain('.barra-lateral.barra-lateral--colapsada nav a')
    expect(estilos).toContain('grid-template-columns: repeat(3, minmax(0, 1fr))')
    expect(estilos).toContain('font-family: "Material Symbols Rounded"')
    ...
    expect(estilos).toContain('content: "sync"')
  })
