...
describe('landing preparada para producción', () => {
  ...
  it('permite alternar ARS y USD con una cotización externa validada', () => {
    ...
    expect(landing).toContain('data-bandera="ARS"')
    expect(landing).toContain('data-bandera="USD"')
    expect(landing).toContain('aria-hidden="true"></span>Pesos')
    ...
    expect(landingJs).toContain('convertirUsdAArs(precioUsd, dolarBlueVenta)')
    expect(landingJs).toContain("let monedaVista = 'USD'")
    expect(estilosLanding).toContain('[data-moneda-plan="ARS"]::before')
    expect(estilosLanding).toContain('[data-moneda-plan="USD"]::before')
